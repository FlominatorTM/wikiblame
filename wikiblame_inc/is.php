<?php
/** WikiBlame
 *
 */
/** Icelandic (íslenska)
 * 
 * See the qqq 'language' for message documentation incl. usage of parameters
 * To improve a translation please visit https://translatewiki.net
 *
 * @ingroup Language
 * @file
 *
 * @author Snævar
 * @author Sveinn í Felli
 */

$messages['January'] = 'Janúar';
$messages['February'] = 'Febrúar';
$messages['March'] = 'Mars';
$messages['April'] = 'Apríl';
$messages['May'] = 'Maí';
$messages['June'] = 'Júní';
$messages['July'] = 'Júlí';
$messages['August'] = 'Ágúst';
$messages['September'] = 'September';
$messages['October'] = 'Október';
$messages['November'] = 'Nóvember';
$messages['December'] = 'Desember';
$messages['ui_lang'] = 'Viðmótstungumál';
$messages['lang'] = 'Tungumál';
$messages['project'] = 'Verkefni';
$messages['article'] = 'Síða';
$messages['needle'] = 'Leitarstrengur';
$messages['skipversions'] = 'Leita með x útgáfu millibili';
$messages['ignorefirst'] = 'Útiloka fyrstu x útgáfur';
$messages['limit'] = 'Útgáfur sem á að athuga';
$messages['start_date'] = 'Upphafsdagur';
$messages['date_format'] = 'MM DD, YYYY';
$messages['revision_date_format'] = '%H:%M, %d. %B %Y';
$messages['order'] = 'Röðun';
$messages['newest_first'] = 'nýjasta fyrst';
$messages['oldest_first'] = 'elsta fyrst';
$messages['binary_search_inverse'] = 'Leita að fjarlægðum texta (eingöngu í tvíunda ham)';
$messages['search_method'] = 'Leitaraðferð';
$messages['binary'] = 'tvíundar';
$messages['linear'] = 'línuleg';
$messages['interpolated'] = 'tvíunda (hraðara og skoðar fleiri útgáfur)';
$messages['ignore_minors'] = 'Útiloka minniháttar breytingar (á tilraunastigi)';
$messages['force_wikitags'] = 'Þvinga leit eftir wiki-texta';
$messages['from_url'] = 'úr vefslóð';
$messages['paste_url'] = 'Límdu vefslóðina á MediaWiki-síðuna';
$messages['no_valid_url'] = 'Þetta er ekki gild MediaWiki-vefslóð';
$messages['start'] = 'Byrja';
$messages['reset'] = 'Endursetja';
$messages['manual'] = 'Leiðbeiningar';
$messages['contact'] = 'Hafa samband';
$messages['get_less_versions'] = 'Leitin gæti farið yfir __NUMREVISIONS__ útgáfur í einu. Til þess að vernda vefþjóninn, þá er eingöngu heimilt að leita að __ALLOWEDREVISIONS__ í einu. Vinsamlegast breyttu stillingunum eða breyttu yfir í tvíundaleit!';
$messages['wrong_skips'] = 'Vitlaus leitarskilyrði: Ef fyrstu útgáfunum __VERSIONSTOSKIP__ er sleppt, þá eru engar útgáfur __VERSIONSTOSEARCH__ eftir til að leita í.';
$messages['search_in_progress_text'] = 'Leita í breytingarskrá _ARTICLELINK_ eftir <b>_NEEDLE_</b> sem einföldum texta';
$messages['search_in_progress_wikitags'] = 'Leita í breytingarskrá _ARTICLELINK_ eftir <b>_NEEDLE_</b> sem wiki-texta';
$messages['no_differences'] = 'Engar breytingar fundust í þeim útgáfum sem leitað var í.';
$messages['inverse_restart'] = 'Engin innsetning eða fjarlæging fundust, var leitarstrengurinn settur inn síðar?';
$messages['inverse_stuck'] = 'Engin innsetning eða fjarlæging fundust í þessum _NUMBEROFVERSIONS_ útgáfum. Var leitarstrengurinn settur inn fyrr?';
$messages['inverse_earliest'] = 'Leita í eldri útgáfum';
$messages['first_version'] = 'Breytingin hlýtur að hafa gerst í fyrstu eða síðustu útgáfu?';
$messages['first_version_present'] = '__NEEDLE__ var þegar til staðar í elstu útgáfu sem leitað var að frá __REVISIONLINK__.';
$messages['earlier_versions_available'] = 'Eldri útgáfur eru líklega til.';
$messages['execution_time'] = 'Leitin tók: _EXECUTIONTIME_ sekúndur';
$messages['versions_found'] = '_NUMBEROFVERSIONS_ útgáfur fundust';
$messages['please_wait'] = 'Bíddu aðeins ...';
$messages['binary_test'] = 'Ber saman mismun útgáfu frá _FIRSTDATEVERSION_ á milli _FIRSTNUMBER_ og _SECONDNUMBER_ sem koma frá _SOURCENUMBER_:';
$messages['dead_end'] = 'Fann dauðan enda (líklega vegna endurtekinna breytinga eða breytingastríðs)';
$messages['once_more'] = 'Einu sinni enn, með tilfinningu:';
$messages['delete_from_here'] = 'Eyði _NUMBEROFVERSIONS_ síðari breytingaútgáfum, því innsetningin hlýtur að hafa verið gerð síðar';
$messages['delete_until_here'] = 'Eyði _NUMBEROFVERSIONS_ síðari breytingaútgáfum, því innsetningin hlýtur að hafa verið gerð fyrr';
$messages['binary_enough'] = 'Gerði nógu margar tilraunir, breytingarskrá síðunnar er í mjög slæmu ásikomulagi, vinsamlegast reyndu að breyta leitarskilyrðum.';
$messages['insertion_found'] = 'Innsetning fannst á milli LEFT_VERSION og RIGHT_VERSION';
$messages['deletion_found'] = 'Fjarlæging fannst á milli LEFT_VERSION og RIGHT_VERSION';
$messages['here'] = 'hér';
$messages['help_translating'] = 'Hjálpa við þýðingu á translatewiki.net';
$messages['start_here'] = 'Leita héðan';
$messages['too_much_versions'] = 'Þú hefur náð hámarksfjölda __VERSIONLIMIT__ útgáfna. Vinsamlegast reyndu aftur eftir __WAITMINUTES__ mínútur eða breyttu yfir í tvíundarleit. Afsakaðu ónæðið.';
$messages['not_found_at_all'] = 'Engar niðurstöður fundust. Athugaðu leitarskilyrðin og reyndu aftur.';
