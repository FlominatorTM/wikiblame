<?php
/** Bangla (বাংলা)
 * 
 * See the qqq 'language' for message documentation incl. usage of parameters
 * To improve a translation please visit https://translatewiki.net
 *
 * @ingroup Language
 * @file
 *
 * @author Aftab1995
 * @author Aftabuzzaman
 * @author Bellayet
 * @author Ehsanulhb
 * @author Elias Ahmmad
 * @author Masumrezarock100
 * @author RIPENDIL
 * @author Rasal Lia
 * @author Sakura286
 * @author Sayma Jahan
 * @author Wikisagnik
 * @author আজিজ
 * @author আফতাবুজ্জামান
 */

$messages['January'] = 'জানুয়ারি';
$messages['February'] = 'ফেব্রুয়ারি';
$messages['March'] = 'মার্চ';
$messages['April'] = 'এপ্রিল';
$messages['May'] = 'মে';
$messages['June'] = 'জুন';
$messages['July'] = 'জুলাই';
$messages['August'] = 'আগস্ট';
$messages['September'] = 'সেপ্টেম্বর';
$messages['October'] = 'অক্টোবর';
$messages['November'] = 'নভেম্বর';
$messages['December'] = 'ডিসেম্বর';
$messages['ui_lang'] = 'ভাষা প্রদর্শন';
$messages['lang'] = 'ভাষা';
$messages['lang_example'] = 'bn, commons, www, …';
$messages['project'] = 'প্রকল্প';
$messages['project_example'] = 'wikipedia, wikisource, wikimedia, wikidata, …';
$messages['tld'] = 'ডোমেইন';
$messages['article'] = 'পাতা';
$messages['needle'] = 'অনুসন্ধান করো';
$messages['skipversions'] = 'সর্বদা x সংস্করণ এড়িয়ে যান';
$messages['ignorefirst'] = 'প্রথম x সংস্করণ উপেক্ষা করুন';
$messages['limit'] = 'সংস্করণ যা পরীক্ষা করতে হবে';
$messages['start_date'] = 'শুরুর তারিখ';
$messages['date_format'] = 'মম দদ, বববব';
$messages['revision_date_format'] = '%H:%M, %d %B %Y';
$messages['order'] = 'ক্রম';
$messages['newest_first'] = 'সর্বশেষ প্রথমে';
$messages['oldest_first'] = 'সর্বপ্রথম প্রথমে';
$messages['binary_search_inverse'] = 'লেখার মুছার জন্য খোঁজ (শুধু বাইনারি)';
$messages['search_method'] = 'অনুসন্ধান পদ্ধতি';
$messages['binary'] = 'বাইনারি';
$messages['linear'] = 'লিনিয়ার';
$messages['interpolated'] = 'বাইনারি (অধিক মুদ্রণ থাকলে দ্রুততর)';
$messages['ignore_minors'] = 'অল্প পরিবর্তনসমূহ উপেক্ষা (পরীক্ষাধীন)';
$messages['force_wikitags'] = 'বলপুর্বক উইকিটেক্সট্ অনুসন্ধান';
$messages['from_url'] = 'ইউআরএল লিংক হতে';
$messages['paste_url'] = 'দয়া করে মিডিয়াউইকির পাতায় url প্রতিলেপন করুন';
$messages['no_valid_url'] = 'এটা কার্যকর MediaWiki ইউঅারএল লিংক নয়';
$messages['start'] = 'শুরু';
$messages['reset'] = 'পুনঃস্থাপন';
$messages['manual'] = 'হস্তচালিত';
$messages['contact'] = 'যোগাযোগ';
$messages['get_less_versions'] = 'আপনি __NUMREVISIONS__ টি পুনর্বিবেচনা একবারে অনুসন্ধান করতে পারবেন। সার্ভার এর সুরক্ষার জন্য, আপনি প্রতি বার সর্বোচ্চ __ALLOWEDREVISIONS__ টি প্রশ্নসম্বলিত অনুসন্ধান করতে পারবেন। দয়া করে সেটিংস পরিবর্তন করুন অথবা বাইনারি উপায়ে অনুসন্ধান চালান।';
$messages['wrong_skips'] = 'ভুল সেটিংস: যদি প্রথমটি _VERSIONTOSKIP_ অনুবাদ গুলো বাদ দেওয়া হয়,তাহলে _VERSIONTOSEARCH_ অনুবাদের কোনটির অনুসন্ধান প্রক্রিয়াগত হবে না।';
$messages['search_in_progress_text'] = '_ARTICLELINK_ এর অনুবাদ ইতিহাস <b>_ NEEDLE</b> এর জন্য সমতল লেখার মত অনুসন্ধান করা হচ্ছে';
$messages['search_in_progress_wikitags'] = '_ARTICLELINK_ এর অনুবাদ ইতিহাস <b>_NEEDLE_</b> এর জন্য উইকি লেখা হিসেবে অনুসন্ধান করা হচ্ছে';
$messages['no_differences'] = 'অনুসন্ধানকৃত সংস্করণে কোন পার্থক্য পাওয়া যায় নি।';
$messages['inverse_restart'] = 'কোন সন্নিবেশ বা সরানো পাওয়া যায় নি, অনুসন্ধান শব্দ কি পরে সন্নিবেশ করা হয়েছে?';
$messages['inverse_stuck'] = ' _NUMBEROFVERSIONS_ এই সংস্করণে কোন সংযোজন বা বিয়োজন পাওয়া যায়নি। অনুসন্ধানের শব্দ কি অপসারণ করা হয়েছিল?';
$messages['inverse_earliest'] = 'আগের সংস্করণে অনুসন্ধান করুন';
$messages['first_version'] = 'পরিবর্তনসমূহ কি প্রথম নাকি সর্বশেষ মুদ্রণে হয়েছিল?';
$messages['first_version_present'] = '__NEEDLE__ ইতিমধ্যেই __REVISIONLINK__ তারিখে অনুসন্ধান করা পুরাতন সংশোধনে উপস্থিত আছে।';
$messages['earlier_versions_available'] = 'সেগুলো সম্ভবত পুরাতন সংশোধন।';
$messages['execution_time'] = 'সম্পন্নের সময়: _EXECUTIONTIME_ সেকেন্ড';
$messages['versions_found'] = '_NUMBEROFVERSIONS_ টি সংস্করণ পাওয়া গেছে';
$messages['please_wait'] = 'অনুগ্রহ করে অপেক্ষা করুন...';
$messages['binary_test'] = '_FIRSTDATEVERSION_  এবং _FIRSTNUMBER_ এবং _SECONDNUMBER_ এর মাঝে পার্থক্য তুলনা করা হচ্ছে যখন _SOURCENUMBER_ থেকে এসেছে:';
$messages['dead_end'] = 'কিছু অপরিবর্তনীয় খুঁজে পাওয়া গেছে (সম্ভবত ফিরে আসা বা সম্পাদনা যুদ্ধের জন্য)';
$messages['once_more'] = 'আরেকবার, অনুভুতির সাথে:';
$messages['delete_from_here'] = 'পর্বের সংস্করণ _NUMBEROFVERSIONS_ অপসারণ করা হচ্ছে যেহেতু তা পরে অবশ্যই অপসারণ করতে হবে।';
$messages['delete_until_here'] = 'সাম্প্রতিক সংস্করণ _NUMBEROFVERSIONS_ অপসারণ করা হচ্ছে যেহেতু সংযোজনা অাগেই সম্পন্ন করতে হবে।';
$messages['binary_enough'] = 'যথেষ্টবার পুনরায় চেষ্টা করা হয়েছে, অনুচ্ছেদ ইতিহাস কিছুটা এলোমেলো, দয়া করে কিছু সেটিংস পরিবর্তনের চেষ্টা করুন।';
$messages['insertion_found'] = 'LEFT_VERSION এবং RIGHT_VERSION এর মাঝে সন্নিবেশ খুঁজে পাওয়া গেছে';
$messages['deletion_found'] = 'LEFT_VERSION এবং RIGHT_VERSION এর মাঝে অপসারণ খুঁজে পাওয়া গেছে';
$messages['here'] = 'এখানে';
$messages['help_translating'] = 'translatewiki.net এ অনুবাদে সাহায্য করুন';
$messages['start_here'] = 'এখানে অনুসন্ধান করুন';
$messages['too_much_versions'] = 'আপনার অনুসন্ধানের সীমা __VERSIONLIMIT__ গুলো মুদ্রণ পরিলক্ষিত। দয়া করে __WAITMINUTES__ মিনিট পর আবার চেষ্টা করুন অথবা বাইনারি অনুসন্ধান এ পরিবর্তন করুন। সাময়িক অসুবিধার জন্য দু:খিত।';
$messages['not_found_at_all'] = 'আপনার অনুসন্ধানের কিছুই পাওয়া যায়নি। সেটিংস পরীক্ষা করুন এবং আবার চেষ্টা করুন।';
