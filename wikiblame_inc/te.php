<?php
/** Telugu (తెలుగు)
 * 
 * See the qqq 'language' for message documentation incl. usage of parameters
 * To improve a translation please visit https://translatewiki.net
 *
 * @ingroup Language
 * @file
 *
 * @author Kiranmayee
 * @author Veeven
 */

$messages['January'] = 'జనవరి';
$messages['February'] = 'ఫిబ్రవరి';
$messages['March'] = 'మార్చి';
$messages['April'] = 'ఏప్రిల్';
$messages['May'] = 'మే';
$messages['June'] = 'జూన్';
$messages['July'] = 'జూలై';
$messages['August'] = 'ఆగష్టు';
$messages['September'] = 'సెప్టెంబరు';
$messages['October'] = 'అక్టోబరు';
$messages['November'] = 'నవంబరు';
$messages['December'] = 'డిసెంబరు';
$messages['ui_lang'] = 'ప్రదర్శనా భాష';
$messages['lang'] = 'భాష';
$messages['project'] = 'ప్రాజెక్టు';
$messages['article'] = 'పేజి';
$messages['needle'] = 'అన్వేషణ';
$messages['start_date'] = 'ఆరంభ తేదీ';
$messages['date_format'] = 'MM DD, YYYY';
$messages['order'] = 'క్రమం';
$messages['newest_first'] = 'కొత్తవి ముందర';
$messages['oldest_first'] = 'పాతవి ముందర';
$messages['search_method'] = 'అన్వేషణా విదానము';
$messages['start'] = 'ఆరంభము';
$messages['please_wait'] = 'దయచేసి వేచివుండండి …';
$messages['help_translating'] = 'translatewiki.net యందు అనువదించడానికి సహాయం చెయ్యండి';
